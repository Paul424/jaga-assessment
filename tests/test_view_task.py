...
def test_tasks_get_2_items(client, task_factory):
    ...
    task2 = task_factory(username="Wilma Flintstone", email="wilma.flintstone@gmail.com")
    response = client.get("/tasks/")
    assert response.status_code == 200
    data = response.get_json()
    assert isinstance(data, list)
...
def test_tasks_create_not_allowed(client, task_factory, method_name):
    ...
    response = getattr(client, method_name)(f"/tasks/{task1.id}", json={
...
def test_tasks_update_item_not_found(client, task_factory):
    task1 = task_factory(username="fred", email="fred.flintstone@gmail.com")
    response = client.put(f"/tasks/{task1.id + 1}", json={
        "username": task1.username, 
        "email": task1.email
    })
...
def test_tasks_delete_item_not_found(client, task_factory):
    task1 = task_factory(username="fred", email="fred.flintstone@gmail.com")
    response = client.delete(f"/tasks/{task1.id + 1}", json={
        "username": task1.username, 
        "email": task1.email
    })
    assert response.status_code == 404
...
def test_tasks_delete_item(client, task_factory):
    task1 = task_factory(username="fred", email="fred.flintstone@gmail.com")
    response = client.delete(f"/tasks/{task1.id}", json={
        "username": task1.username, 
        "email": task1.email
    })
